...
def LF_common_1000(c):
    if (c[1].get_span() in common1000):
        return -1


def LF_common_2000(c):
    if (c[1].get_span() in common2000):
        return -1
    else:
        return 0

def LF_is_in_typeDatabase(c):
    print c[0]
...
def LF_roman_numeral(c):
    ...
    if re.match(
            r'((?<=\s)|(?<=^))(M{1,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})|M{0,4}(CM|CD|D?C{1,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})|M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{1,3})(IX|IV|V?I{0,3})|M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{1,3}))(?=\s)',
            biomarker):
        return -1
...
def LF_check_medData(c):
    if(c.medium.get_span() in medData):
        return 1
    else:
        return 0
#def LF_same_thing(c):
